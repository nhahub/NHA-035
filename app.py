...
@st.cache_resource
def load_model():
    model_path = r"D:\marbert_model\model-20251031T091239Z-1-001\model"

    tokenizer_type = AutoTokenizer.from_pretrained(model_path)

    model = BertForSequenceClassification.from_pretrained(model_path, device_map=None)

    pipeline_model = pipeline(
        "sentiment-analysis", model=model, tokenizer=tokenizer_type
    )
