...
def extract_batch(
    ...
    This version is enhanced for logging and robustness.
    """

    logger.info(f"Starting extraction for batch {batch_number} from file '{file_name}'")
    ...
    try:
        ...
        # Read the actual batch
        batch_df = pd.read_csv(
            file_path,
            header=None,  # No header in chunk
            names=header,  # Apply correct columns
            skiprows=start_row,
            nrows=batch_size,
        )
